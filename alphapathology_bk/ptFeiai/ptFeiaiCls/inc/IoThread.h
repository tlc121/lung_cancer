...
namespace ALPHA
{
	namespace FEIAI
	{
		...
		public:
			explicit IOThread() {}
			explicit IOThread(IFPipelinePathologyFEIAIIn* FEIAI_input, openslide_t* slide, int cut_width, int cut_height) : m_FEIAI_input(FEIAI_input), m_slide(slide), m_cutWidth(cut_width), m_cutHeight(cut_height) { m_slide_name = m_FEIAI_input->m_slide_name; }
			~IOThread() {}
			patch get_patch();
			void startThread();
		...
		private:
			IFPipelinePathologyFEIAIIn* m_FEIAI_input;
			std::mutex m_mutex;
