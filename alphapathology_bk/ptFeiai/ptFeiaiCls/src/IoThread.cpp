...
void ALPHA::FEIAI::IOThread::startThread()
{
	std::thread* t = new std::thread(std::bind(&IOThread::run, this));
	m_run_thread = t;
}

void  ALPHA::FEIAI::IOThread::run()
{
...
void  ALPHA::FEIAI::IOThread::add_patch(patch&& p)
{
	std::lock_guard<std::mutex> g(m_mutex);
	m_patchList.emplace_back(p);
}

ALPHA::FEIAI::patch ALPHA::FEIAI::IOThread::get_patch()
{
	std::lock_guard<std::mutex> g(m_mutex);
	ALPHA::FEIAI::patch p = m_patchList.front();
	m_patchList.pop_front();
	return std::move(p);
}

int ALPHA::FEIAI::IOThread::get_patchlist_size()
{
