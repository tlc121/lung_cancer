...
namespace ALPHA
{
	namespace XS {
		...
		cv::Mat normalize_image(const cv::Mat& in_img)
		{
			...
			cv::Mat norm_img = cv::Mat(rows, cols, CV_32FC3);
		...
		void ALPHA::XS::AIThread::startThread()
		{
			std::thread t(std::thread(std::bind(&AIThread::run, this)));
			t.join();
		}

		void ALPHA::XS::AIThread::run()
		{
			// step 0: parse config
			libconfig::Config config;
			COMM::loadCfg(m_configPath, config);
			const libconfig::Setting& root = config.getRoot();
			const libconfig::Setting& shape = root["XS_PREPROCESS"]["CLS_INPUT_SHAPE"];
			const int width = shape[0];
			const int height = shape[1];

			const int batch_size = config.lookup("XS_CLS.BATCH_SIZE");
			const int num_tumor_thres = config.lookup("XS_CLS.NUM_PATCH_THRESHOLD");
			const float tumor_prob_thres = config.lookup("XS_CLS.PROB_CLS_THRESHOLD");
			const std::string model_path_normal_disease = config.lookup("XS_CLS.MODEL_CLS_PATH");
			const std::string input_node_normal_disease = config.lookup("XS_CLS.INPUT_CLS_NODE");
			const std::string output_node_normal_disease = config.lookup("XS_CLS.OUTPUT_CLS_NODE");
			const std::string normal_disease_type_0 = config.lookup("XS_CLS.TYPE_0");
			const std::string normal_disease_type_1 = config.lookup("XS_CLS.TYPE_1");
					...
					for (int y = 0; y < height; y++)
					{
						...
						for (int x = 0; x < width; x++)
						{
							...
							for (int c = 0; c < depth; c++)
							{
								...
								input_tensor_mapped(imgidx_batch, 2-c, y, x) = *dataValue;
							}
						}
					}
