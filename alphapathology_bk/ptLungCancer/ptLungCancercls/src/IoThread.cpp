...
void ALPHA::LungCancer::IOThread::startThread()
{
	std::thread* t = new std::thread(std::bind(&IOThread::run, this));
	m_run_thread = t;
}

void  ALPHA::LungCancer::IOThread::run()
{
	...
	std::cout << width_num << std::endl;
	std::cout << height_num << std::endl;

	//this->m_num_patches = width_num * height_num;
	for (int h_idx = 0; h_idx < height_num; h_idx++)
	{
		for (int w_idx = 0; w_idx < width_num; w_idx++)
		{

			if (this->get_patchlist_size() > 500) {
				::Sleep(3000);
			}
	...
	m_ratio_blur = m_num_blur * 1.0 / (m_num_images);
	m_flag = true; // cut svs finish
}

void  ALPHA::LungCancer::IOThread::add_patch(patch&& p)
{
	std::lock_guard<std::mutex> g(m_mutex);
	m_patchList.emplace_back(p);
}

ALPHA::LungCancer::patch ALPHA::LungCancer::IOThread::get_patch()
{
	std::lock_guard<std::mutex> g(m_mutex);
	ALPHA::LungCancer::patch p = m_patchList.front();
	m_patchList.pop_front();
	return std::move(p);
}

int ALPHA::LungCancer::IOThread::get_patchlist_size()
{
