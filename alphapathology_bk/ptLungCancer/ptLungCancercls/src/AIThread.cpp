...
namespace ALPHA
{
	namespace LungCancer {
		...
		cv::Mat normalize_image(const cv::Mat& in_img)
		{
			...
			for (int i = 0; i < rows; i++) {
				for (int j = 0; j < cols; j++) {
					...
					float replace_b = (blue / 255.0 - 0.406) / 0.225;
					float replace_g = (green / 255.0 - 0.456) / 0.224;
					float replace_r = (red / 255.0 - 0.485) / 0.229;
		...
		void ALPHA::LungCancer::AIThread::startThread()
		{
			std::thread t(std::thread(std::bind(&AIThread::run, this)));
			t.join();
		}

		void ALPHA::LungCancer::AIThread::run()
		{
			...
			const libconfig::Setting& shape = root["LungCancer_PREPROCESS"]["CLS_INPUT_SHAPE"];
			const int width = shape[0];
			const int height = shape[1];

			const int batch_size = config.lookup("LungCancer_CLS.BATCH_SIZE");
			const int num_tumor_thres = config.lookup("LungCancer_CLS.NUM_PATCH_THRESHOLD");
			const float tumor_prob_thres = config.lookup("LungCancer_CLS.PROB_CLS_THRESHOLD");
			const std::string model_path_normal_disease = config.lookup("LungCancer_CLS.MODEL_CLS_PATH");
			const std::string input_node_normal_disease = config.lookup("LungCancer_CLS.INPUT_CLS_NODE");
			const std::string output_node_normal_disease = config.lookup("LungCancer_CLS.OUTPUT_CLS_NODE");
			const std::string normal_disease_type_0 = config.lookup("LungCancer_CLS.TYPE_0");
			const std::string normal_disease_type_1 = config.lookup("LungCancer_CLS.TYPE_1");
